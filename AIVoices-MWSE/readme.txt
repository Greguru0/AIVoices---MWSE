AI Voices - V1.01a
AI Voices is an MWSE mod for Morrowind that generates spoken dialogue for NPCs in real time using local or online text-to-speech engines. When an NPC speaks. Support for XTTS, Piper and ElevenLabs.
...
Pronunciation dictionary - customize how specific words and names are spoken
...
Install size: approximately 7GB including the model and dependencies
...
Install size: approximately 214MB with the generic sample voice
...
Extract the contents of this zip folder into the Morrowind Data Files folder so that Data Files\MWSE\mods\AIVoices\ exists
Run install_aivoices.bat from Data Files\MWSE\mods\AIVoices\
Follow the installer prompts to select which voice engines to set up (You can set up more than one)
Launch Morrowind - AI Voices will start automatically. XTTS will take a moment to fire up as it loads the model when it's first initialized on start. There will be an on-screen message that displays when its ready.

The installer creates a local Python virtual environment, downloads required dependencies, builds XTTS reference samples from your installed voice files, and writes all runtime and settings files into Data Files\AIVoicesBackend\.
The installer does not modify any original Morrowind files.
Some empty backend folders contain .gitkeep files so GitHub preserves the intended folder structure.

Uninstallation
Run uninstall_aivoices.bat from Data Files\MWSE\mods\AIVoices\. This removes the virtual environment, runtime files, settings, and engine-specific folders. It does not remove the mod's Lua files, the installer, or your Morrowind voice files.
...
Actor-specific - if the actor's ID or name matches a key in the voice map file, that voice is used
Race and gender fallback - if no actor-specific match is found, the race and gender combination is used (e.g. darkElfMale, nordFemale). Note that some special NPCs will not fallback to race/gender, as they are "Creatures" (Vivec, for example).
...
Pronunciation
A pronunciation dictionary at AIVoicesBackend\settings\pronunciation.txt allows you to control how specific words are spoken. Entries use the format original=replacement. Longer entries are matched before shorter ones to avoid partial substitutions.
...
Configure XTTS generation parameters (temperature, repetition penalty, top K, top P)
Adjust speech speed (applies to all engines - ElevenLabs capped at 1.2 by API)
...
Watcher process settings (console visibility, Python path, heartbeat interval)
...
Data Files\
  ...
  AIVoicesBackend\
    ...
    build_reference_samples.py  - used by the installer to create XTTS reference samples from Morrowind voice files; advanced users can also run it manually to create custom XTTS samples.
...
XTTS can be slow on weak hardware. Generation time depends on CPU/GPU performance. On CPU-only systems, a single line may take 30 seconds or more. Piper or ElevenLabs are recommended for users without a capable NVIDIA GPU.
XTTS line caching has limited benefit. Because Morrowind dialogue is large and varied, the same line is rarely repeated outside of common greetings. Caching is available but disabled by default.
Piper requires user-provided voice files. The generic sample voice downloaded during install is not Morrowind-specific. Morrowind-appropriate voices require sourcing compatible .onnx files separately.
ElevenLabs uses credits per line. Every uncached dialogue line sent to ElevenLabs consumes API credits. This is noted during install and in the MCM.
